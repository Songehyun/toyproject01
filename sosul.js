...
let count = 0;
let many = [];
let blob = new Blob([sosulText]);
let sizeInBytes = blob.size;

for (i = 0; i < sosulText.length; i++) {
  if (sosulText[i] === '송') {
    if (sosulText[i + 1] === '이') {
      if (sosulText[i + 2] === '현') {
        count = count + 1;
...
console.log(
  `1. 위 sosulText 라는 문자열의 데이터 크기는 몇입니까? : ${sizeInBytes}`,
);
// Blob 쓴다.
console.log(
  "2. 위 sosulText 에 존재하는 '나' 의 이름은 몇번 언급되었습니까? : ",
  count,
);
// 텍스트에서 '송이현' 찾아서 뒤에 숫자 1~ 붙여서 배열이든 뭐든 빼면됨
// 텍스트문자열 순회해서 i = 송 i+1 = 이 i+2 = 현 만족하면 카운트+1해서 세기
console.log(`3. 가장 많이 언급된 단어는 무엇입니까? :`);
// 텍스트 띄어쓰기를 기준으로 배열로 다 뺄수 있나? 그리고 중복되는거 몇개씩인지 셀수 있나? join?
console.log(
  '4. 극중 나의 배역이 마음에 들지 않습니다. 다른이름으로 배역을 변경해주세요 :',
);
// 내 이름을 바꿀 배역 이름으로 and 바꿀 배역 이름을 내이름으로
console.log(
  '5. 극중에 가장 나쁜사람은 누구입니까? : 악당 공욱재 슬픈 IT 세계를 만들어 행복한 IT 세계를 파괴하려고 하는게 주 목적이기 때문에',
);
// 글 읽어보기
